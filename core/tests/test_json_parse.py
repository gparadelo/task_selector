...
from core.selector.JsonParse import _parse_json_task, _parse_json_task_list, parse_task_list
...
class JsonParseTest(TestCase):
    ...
    def test_parse_json_task_list(self):
        ...
        self.assertEqual(_parse_json_task_list(json_task_list), [task1, task2])
    
    def test_parse_task_list(self):
        task_list = "[{'name': 'task1', 'resources':['resource1'], 'profit':2.9}, " + \
            "{'name': 'task2', 'resources':['resource2'], 'profit':6}]"
        task1 = Task('task1', {Resource('resource1')}, 2.9)
        task2 = Task('task2', {Resource('resource2')}, 6)
        self.assertEqual(parse_task_list(task_list), [task1, task2])
